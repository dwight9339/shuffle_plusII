...
module.exports.spread = (tracks, shuffleParam) => {
    if (!tracks) return;
    
    let trax = simplifyTracks(tracks);
    let arrays = getUnique(trax, shuffleParam).map(param => trax.filter(track => track[shuffleParam] === param));
...
module.exports.biased = (tracks, shuffleParam, biasOptions) => {
    if (!tracks) return;
